...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.error || 'Senha inválida');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-vanilla-white flex items-center justify-center p-6 relative overflow-hidden select-none">
      ...
      <motion.div
        ...
        className="w-full max-w-sm bg-daisy-white border border-rose-cream/45 p-8 rounded-3xl shadow-xl space-y-6 relative z-20"
      >
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 bg-golden-honey hover:bg-honey-yellow hover:text-soft-brown text-white font-bold rounded-xl transition duration-150 cursor-pointer flex items-center justify-center gap-2 text-xs uppercase tracking-wider disabled:opacity-50"
          >
